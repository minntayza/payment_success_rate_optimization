...
export default function Home() {
  ...
  if (!admin) return <BankPortal email={email} headers={headers} onSignOut={signOut} />;
...
function BankPortal({ email, headers, onSignOut }: { email: string; headers: Record<string, string>; onSignOut: () => void }) {
  const [overview, setOverview] = useState<{ customer_name: string; accounts: { name: string; number: string; balance: number; currency: string }[]; beneficiaries: { name: string; initials: string; account_number: string }[]; activity: { id: string; name: string; category: string; amount: number; date: string; icon: string }[]; notifications: number } | null>(null);
  const [recipient, setRecipient] = useState(""); const [accountNumber, setAccountNumber] = useState(""); const [amount, setAmount] = useState(""); const [notice, setNotice] = useState("");
  const load = async () => { const response = await fetch(`${api}/api/customer/overview`, { headers }); setOverview(await response.json()); };
  useEffect(() => { load().catch(() => setNotice("Unable to load your account.")); }, []);
  const transfer = async (event: FormEvent) => { event.preventDefault(); const response = await fetch(`${api}/api/customer/transfers`, { method: "POST", headers: { ...headers, "Content-Type": "application/json" }, body: JSON.stringify({ recipient, account_number: accountNumber, amount: Number(amount) }) }); const result = await response.json(); if (!response.ok) { setNotice(result.detail); return; } setNotice(`Transfer scheduled. Reference: ${String(result.reference).slice(0, 8)}`); setRecipient(""); setAccountNumber(""); setAmount(""); await load(); };
  return <main className="bank-shell"><header className="bank-top"><div className="bank-brand"><span>◒</span> union bank</div><nav><a>Home</a><a>Accounts</a><a>Payments</a><a>Cards</a></nav><div><button className="bell">● {overview?.notifications ?? 0}</button><button className="profile" onClick={onSignOut}>{email.slice(0, 1).toUpperCase()}</button></div></header><section className="bank-content"><p className="eyebrow">GOOD MORNING</p><h1>Hello, {overview?.customer_name ?? "there"}.</h1><p className="bank-subtitle">Here is your financial snapshot for today.</p><section className="account-grid">{overview?.accounts.map((account, index) => <article className={`account-card card-${index}`} key={account.number}><span>{account.name}</span><strong>{account.currency} {account.balance.toLocaleString(undefined, { minimumFractionDigits: 2 })}</strong><small>{account.number}</small></article>)}<button className="new-account">＋<br /><span>Open an account</span></button></section><section className="bank-grid"><article className="activity-card"><div className="section-title"><div><h2>Recent activity</h2><p>Latest account movements</p></div><button className="link-button">View all</button></div>{overview?.activity.map((item) => <div className="activity" key={item.id}><i>{item.icon}</i><div><b>{item.name}</b><span>{item.category} · {item.date}</span></div><strong className={item.amount < 0 ? "debit" : "credit"}>{item.amount < 0 ? "−" : "+"}${Math.abs(item.amount).toFixed(2)}</strong></div>)}</article><article className="transfer-card"><p className="eyebrow">QUICK TRANSFER</p><h2>Send money</h2><form onSubmit={transfer}><label>Recipient<input value={recipient} onChange={(event) => setRecipient(event.target.value)} placeholder="Recipient name" required /></label><label>Account number<input value={accountNumber} onChange={(event) => setAccountNumber(event.target.value)} placeholder="Account number" required /></label><label>Amount (USD)<input type="number" step="0.01" min="1" value={amount} onChange={(event) => setAmount(event.target.value)} placeholder="0.00" required /></label><button>Continue transfer →</button></form>{notice && <p className="transfer-notice">{notice}</p>}</article></section><section className="beneficiary-card"><div className="section-title"><div><h2>Send again</h2><p>Your saved people</p></div><button className="link-button">Manage</button></div><div className="beneficiaries">{overview?.beneficiaries.map((person) => <button key={person.account_number} onClick={() => { setRecipient(person.name); setAccountNumber(person.account_number.replace("•••• ", "")); }}><i>{person.initials}</i><b>{person.name}</b><span>{person.account_number}</span></button>)}</div></section></section></main>;
}
